...
try:
    ...
    target_coords = np.ones((3, 3))
    target_coords[1:] = np.array([[0.15, 0.5,  0.85], [0.3,  0.5,  0.2 ]])
    target_points = alg2d.vector(target_coords).dual()

    coordinates = np.ones((3, 5))
    coordinates[1:] = np.array([
        0.476018975165776,
        0.6260100093482048,
        0.8753453209080457,
        0.5839114813135214,
    ...
    points = alg2d.vector(coordinates).dual()
    pairs = [(0, 1), (1, 2), (2, 3), (3, 4)]
    def graph_mechanism_func():
        return [
            0xffffff, ~alg2d.evenmv(e=1, e01=0., e02=0.25) >> alg2d.blades.e12,
            0x000000, "Live demo involved closed source code which is not available with this",
            0xffffff, ~alg2d.evenmv(e=1, e01=0., e02=0.2) >> alg2d.blades.e12,
            0x000000, "slideset, please watch the recording on bivector for this part.",
            # *graph_guess_func()
        ]
else:
